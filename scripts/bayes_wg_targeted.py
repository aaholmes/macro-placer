"""TARGETED-diffusion variant of bayes_window_greedy: the Langevin kick is gated to
ONLY overlapping macros (sigmoid((local_density-1)/0.5)), so isolated/settled macros
are left alone -- global diffusion hurt by jiggling the ~99% that aren't overlapping.
Everything else (objective, seeds, pool) is identical, for a clean A/B vs global.
...
DB = "sqlite:////home/laz/partcl/my-macro-placer/notes/bayes_wgt.db"
ITERS = 5000
...
def hp_from(p):
    return dict(lam_wl0=p["lam_wl0"], lr=p["lr"], tau_d=p["tau_d"], iters=ITERS,
                ...
                diff_D0=p.get("diff_D0", 0.0))


def diff_place(P, seed, hp):
    """GPU differentiable stage with the overlap-ramp window (raw t)."""
    ...
    cap = 1.0
    D0 = hp.get("diff_D0", 0.0)                      # Langevin diffusion magnitude (0 = off)
    ngen = torch.Generator(device=P.dev).manual_seed(seed * 1000 + 7) if D0 > 0 else None
    if D0 > 0:                                       # Stokes-Einstein mobility ~ 1/sqrt(area): big macros diffuse less
    ...
    for it in range(hp["iters"]):
        ...
        opt.step()
        with torch.no_grad():
            if D0 > 0 and w < 1.0:                  # TARGETED diffusion: kick ONLY overlapping macros
                R, C = rho.shape                    # sample each macro's local density (reuse rho)
                col = (coord[:, 0] / P.W * C).long().clamp_(0, C - 1)
                row = (coord[:, 1] / P.H * R).long().clamp_(0, R - 1)
                gate = torch.sigmoid((rho.detach()[row, col] - 1.0) / 0.5).unsqueeze(1)  # ~1 if >1 layer, ~0 if isolated
                coord.add_(D0 * csz * (1.0 - w) * mob * gate * torch.randn(coord.shape, generator=ngen, device=P.dev))
            coord[:, 0].clamp_(P.hw, P.W - P.hw); coord[:, 1].clamp_(P.hh, P.H - P.hh)
    out = coord.detach()
    if not torch.isfinite(out).all():               # guard: a too-large D0 can blow up -> fail this trial gracefully
        raise FloatingPointError("diffusion produced non-finite coords")
...
def objective(trial):
    ...
    p = dict(ov_hold=trial.suggest_float("ov_hold", 0.0, 0.5),          # free (new)
             ...
             diff_D0=trial.suggest_float("diff_D0", 0.0, 0.03))         # Langevin diffusion (0 = off; 1/sqrt(area) mobility, decays with 1-w)
    try:
...
def finalize(study, top_k=6, n_seeds=16):
    ...
    json.dump({"benches": BENCHES, "reference": REF, "greedy_moves": GREEDY_MOVES, **best},
              open(f"{ROOT}/notes/bayes_wgt_final.json", "w"), indent=2)
...
def main():
    ...
    study = optuna.create_study(study_name="diffplace_wgt", storage=DB, direction="minimize",
                                load_if_exists=True,
    ...
    if len(study.trials) == 0:                       # warm-start (mirror bayes_window)
        ...
        def enq(params):                             # pin diffusion OFF for known-good seeds unless set
            params = dict(params); params.setdefault("diff_D0", 0.0); _enq(params)
        ...
        try:
            ...
            # PAIRED diffusion: same top-6 window configs, now with nonzero D0 (spread over a
            # small grid) -> clean (config, D0=0) vs (config, D0>0) contrast for attribution.
            DG = [0.004, 0.008, 0.012, 0.016, 0.020, 0.024]
            for i, t in enumerate(wtop[:6]):
                p = {k: t.params[k] for k in WK}
                p["lr"] = min(p["lr"], 0.5); p["diff_D0"] = DG[i % len(DG)]
                enq(p)
            print(f"seeded {len(wtop)} window configs at D0=0 + {min(6,len(wtop))} paired at D0>0 "
        ...
        for d in DIV:
            enq({**base_lock, **d})
        # diffusion seeds: 221 manifold + window + nonzero Langevin D0 (small/mid) so TPE has
        # non-zero-diffusion anchors to explore the barrier-crossing / diversity region.
        for D0 in (0.006, 0.015):
            enq({**base_lock, "gamma": P0["gamma"], "lam_wl0": P0["lam_wl0"], "lr": 0.30,
                 "ov_hold": 0.19, "ov_ramp": 0.58, "diff_D0": D0})
