...
DB = "sqlite:////home/laz/partcl/my-macro-placer/notes/bayes_wg.db"
ITERS = 5000
...
def diff_place(P, seed, hp):
    ...
    for it in range(hp["iters"]):
        ...
        opt.step()
        with torch.no_grad():
            if D0 > 0 and w < 1.0:                  # annealed diffusion: on during the free window, 0 once overlap penalty full
                coord.add_(D0 * csz * (1.0 - w) * mob * torch.randn(coord.shape, generator=ngen, device=P.dev))
...
def finalize(study, top_k=6, n_seeds=16):
    ...
    json.dump({"benches": BENCHES, "reference": REF, "greedy_moves": GREEDY_MOVES, **best},
              open(f"{ROOT}/notes/bayes_wg_final.json", "w"), indent=2)
...
def main():
    ...
    study = optuna.create_study(study_name="diffplace_wg", storage=DB, direction="minimize",
                                load_if_exists=True,
