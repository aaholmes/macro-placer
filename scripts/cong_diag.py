...
ap.add_argument("--te-ramp", action="store_true", help="ramp congestion on te=t^ramp_p (like density) not linear t")
a = ap.parse_args()
nm = a.bench
...
for lc in [float(x) for x in a.lam_cs.split(",")]:
    def loss(Pl, coord, t):
        ...
        if lc > 0:
            ...
            ramp = (t ** p["ramp_p"]) if a.te_ramp else t            # te-ramp: on only when clustered
            L = L + lc * ramp * term
        return L
